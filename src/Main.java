...
public class Main {
    public static void main(String[] args) {
        ...
        List<Product> listaDeiBoys = products.stream()
                .filter(product -> product.getCategory().equals("Boys"))
                .toList();

        System.out.println(listaDeiBoys);

        List<Product> listaDeiBoysConSconto = products.stream()
                .filter(product -> product.getCategory().equals("Boys"))
                .toList();

        listaDeiBoysConSconto.forEach(product -> product.setPrice(Math.floor((product.getPrice() * 0.9) * 100) / 100));
        // prndo il 90% del prezzo al posto di sottrarre il 10% e ci faccio un Match.floor per arrotondarlo alla seconda cifra decimale
        // perche mi usciva fino alla terza cifra decimale cosi come ho fatto nella classe Product

        System.out.println(listaDeiBoysConSconto);
